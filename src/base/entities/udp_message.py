...
class UdpMessage(Entity):
    ...
    def get_udp_message(self, *args):
        ...
        """
        (latitude, longitude, bearing, velocity, accuracy, id_) = args
        self.data[CLIENT_DATA] = dict()
        self.data[CLIENT_DATA][TIMESTAMP] = Utils.get_times()
        self.data[CLIENT_DATA][ID] = id_
    ...
    def get_udp_message_proto(*args):
        ...
        data = request.clientLocationData
        data.debugMode = True
        data.notifyMe = True
        data.notifyOthers = True

        client_data = data.clientData
        client_data.id = id_
        client_data.clientDataType = ClientData_pb2.ClientData.CAR
        client_data.latitude = latitude
        client_data.longitude = longitude
        client_data.bearing = bearing
