...
@allure.description("""
    Functional tests.
    1. Check that service responded on 'GetHash' request properly.
    2. Check that service response contains desired properties.
    """)
@allure.severity(allure.severity_level.BLOCKER)
@allure.testcase(BaseConfig.GITLAB_URL + "tests/regression_tests/areas_blacklist_service_tests/get_hash_test.py",
                 "TestGetHash")
...
class TestGetHash(object):
    ...
    def test_get_hash_method_works(self):
        response_ = ApiClient().areas_blacklist_svc.get_hash()
    ...
    def test_attributes_in_get_hash_method(self):
        response_ = ApiClient().areas_blacklist_svc.get_hash()[0]
