...
class TestGetRemoteConfigs:

    @automation_logger(logger)
    def test_get_remote_configs_method_works(self, api_client):
        allure.step("Verify that response is not empty and status code is 200")
        _response = api_client.remote_config_svc.get_configs()
    ...
    @automation_logger(logger)
    def test_attributes_in_get_remote_configs(self, api_client):
        allure.step("Verify response properties and that 'data' is dict object.")
        _response = api_client.remote_config_svc.get_configs()[0]
